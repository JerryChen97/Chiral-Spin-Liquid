...
model_params = {
    'Lx': 1, 'Ly': 4, 'bc':"periodic",# Ly is set below
    'bc_MPS': "infinite",
    'verbose': 1,
    'cons_N': 'N',
    'cons_Sz': 'Sz',
...
class FermionicChiral(CouplingMPOModel):
    ...
    def init_terms(self, model_params):
        ...
        td = np.asarray(model_params.get('td', 0.5j))
        ...
        for u1, u2, dx in self.lat.pairs['next_next_nearest_neighbors']:
            self.add_coupling(td, u1, 'Cdu', u2, 'Cu', dx, 'JW', True)
            self.add_coupling(td, u1, 'Cdd', u2, 'Cd', dx, 'JW', True)
